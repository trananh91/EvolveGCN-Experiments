...
class Link_Pred_Tasker():
	...
	# def get_node_feats_helper(self,args,dataset, adj):
	...
	# 	# 	self.feats_per_node = max_deg_out + max_deg_in
	# 		def get_node_feats(adj):
	# 			return get_node_feats_helper(self,args,dataset, adj)
	...
	# def get_node_feats_helper(adj, self=None, args=None, dataset=None):
	# 	if args.use_2_hot_node_feats:
	# 		max_deg_out, max_deg_in = tu.get_max_degs(args,dataset)
	# 		self.feats_per_node = max_deg_out + max_deg_in
	# 		return tu.get_2_hot_deg_feats(adj,
	...
	# def get_node_feats(adj, self=None, args=None, dataset=None):
	# 	return get_node_feats_helper(adj)
	...
	# 			# self.get_node_feats = tu.get_2_hot_deg_feats(adj,
	# 			# 							  max_deg_out,
	# 			# 							  max_deg_in,
	# 			# 							  dataset.num_nodes)
	...
	# 			# self.get_node_feats = tu.get_1_hot_deg_feats(adj,
	# 			# 							  max_deg,
	# 			# 							  dataset.num_nodes)
	...
	# 			# self.get_node_feats = dataset.nodes_feats

	# 	return get_node_feats

	# global max_deg_out, max_deg_in, num_nodes
		...
		if args.use_2_hot_node_feats:
			...
			def get_node_feats(adj):
				return tu.get_2_hot_deg_feats(adj,
											  ...
											  max_deg_in,
											  dataset.num_nodes)
			# get_node_feats = get_node_feats(adj)
	...
	def get_sample(self,idx,test, **kwargs):
		...
		# label_adj['idx'] = torch.cat([label_adj['idx'],non_exisiting_adj['idx'].type(torch.FloatTensor)])
		label_adj['idx'] = torch.cat([label_adj['idx'],non_exisiting_adj['idx'].type(torch.LongTensor)])
